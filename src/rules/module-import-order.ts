  ...
  create(context) {
    ...
    const optionsEntries = options.map(item => {
      ...
      const entries = [
        ...
        ...Object.entries(after).map(([k, v]) => [v, +k + afterOffset] as const),
      ]
    ...
    return {
      ImportDeclaration(node) {
        ...
        const importedMembers = specifiers
          .filter(item => item.type === 'ImportSpecifier') as ImportSpecifier[]
        ...
        context.report({
          ...
          fix(fixer) {
            ...
            return importedMembers
              .map((item, i) => fixer.replaceText(item, orderedImports[i]))
          },
        })
      },
    }
